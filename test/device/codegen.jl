...
#
# Cells known to fail are listed in `SKIPPED` with the reason.
...
# Cells that do not compile today, with the reason. These are limitations of the Khronos
# translator the LTS stack uses (see `_compiler_config`); the LLVM SPIR-V back-end compiles
# every cell. The library-based translator exits the process on these errors instead of
# throwing, so the cells are skipped rather than marked broken, and an entry that starts
# compiling again goes unnoticed: re-check the list when bumping SPIRV_LLVM_Translator_jll.
const SKIPPED = Set{Tuple{String, DataType}}(
    if oneL0.LTS[]
        [
...
function smoke(label, f, T)
    @testset "$label($T)" begin
        if (label, T) in SKIPPED
            @test_skip false
        else
            R = result_type(f, T)
            @test compiles(smoke_kernel, Tuple{typeof(f), DevVec{R}, DevVec{T}})
        end
    end
    return
end

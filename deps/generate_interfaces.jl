  ...
  for (i, header) in enumerate(headers)
    ...
    if version == 'W'
      ...
      if isempty(list_versions)
        ...
        if name_routine ∉ ("init_matrix_handle", "init_matmat_descr", "release_matmat_descr", "set_matmat_data", "set_csr_data", "geqrf")
          header = replace(header, "void onemkl" => "int onemkl")
        end
        if library == "sparse"
...
function generate_cpp(library::String, filename::String, output::String)
  ...
  for (header, name, version, template) in signatures
    ...
    if template
      ...
      if name ∉ ("init_matrix_handle", "init_matmat_descr", "release_matmat_descr", "set_matmat_data", "set_csr_data", "geqrf")
    ...
    if occursin("scratchpad_size", name)
      ...
      (name ∉ ("init_matrix_handle", "init_matmat_descr", "release_matmat_descr", "set_matmat_data", "set_csr_data", "geqrf")) && write(oneapi_cpp, "   __FORCE_MKL_FLUSH__(status);\n")
      (name ∉ ("init_matrix_handle", "init_matmat_descr", "release_matmat_descr", "set_matmat_data", "set_csr_data", "geqrf")) && write(oneapi_cpp, "   return 0;\n")
    end
    write(oneapi_cpp, "}")
    write(oneapi_cpp, "\n\n")
  end

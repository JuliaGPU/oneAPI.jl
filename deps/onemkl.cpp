...
extern "C" int onemklHgemm(syclQueue_t device_queue, onemklTranspose transA,
                           ...
                           int64_t k, sycl::half alpha, const sycl::half *A,
                           int64_t lda, const sycl::half *B, int64_t ldb,
                           sycl::half beta, sycl::half *C, int64_t ldc) {
  oneapi::mkl::blas::column_major::gemm(device_queue->val, convert(transA),
                                        convert(transB), m, n, k, alpha, A, lda,
                                        B, ldb, beta, C, ldc);
  return 0;
}
...
// oneMKL keeps a cache of SYCL queues and tries to destroy them when unloading
// the library. that is incompatible with oneAPI.jl destroying queues before
// that, so expose a function to manually wipe the device cache when we're
// destroying queues.
...
extern "C" void onemklDestroy() { oneapi::mkl::gpu::clean_gpu_caches(); }

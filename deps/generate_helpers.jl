...
"sort_matrix", "optimize_gemv", "optimize_trmv", "optimize_trsv", "optimize_trsm"]
...
function analyzer_template(library::String, cpp_headers::String, name_routine::String)
  ...
  if library ∈ ("blas", "sparse") && !(name_routine ∈ non_parametric_routines)
    (library == "blas") && (prefix = "BUF_")
    (library == "sparse") && (prefix = "SPARSE_")
    ...
    for (type, version, suffix) in [(["float"], "S", ""),
                                    (["double"], "D", ""),
                                   (["std::complex<float>"], "C", ""),
    ...
    for (type, version, suffix) in [(["float","float","float"], "S", ""),
    ...
    for (type, version, suffix) in [(["float","float","float","float"], "S", ""),

...
for (fname, elty) in
    ...
    @eval begin
        function gemm_batched!(transA::Char,
            ...
            bsize = length(A)
            m_dev = Vector{Int}(fill(m,bsize))
            n_dev = Vector{Int}(fill(n,bsize))
            k_dev = Vector{Int}(fill(k,bsize))
            lda_dev = Vector{Int}(fill(lda,bsize))
            ldb_dev = Vector{Int}(fill(ldb,bsize))
            ldc_dev = Vector{Int}(fill(ldc,bsize))
            alpha_dev = Vector{$elty}(fill(alpha,bsize))
            beta_dev = Vector{$elty}(fill(beta,bsize))
            groupsize_dev = Vector{Int}(fill(1,bsize))

...
for (fname, elty) in
    ...
    @eval begin
        function gemm_batched!(transA::Char,
            ...
            bsize = length(A)
            m_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            n_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            k_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            lda_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            ldb_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            ldc_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)
            alpha_buf = oneVector{$elty,oneL0.SharedBuffer}(undef,bsize)
            beta_buf = oneVector{$elty,oneL0.SharedBuffer}(undef,bsize)
            groupsize_buf = oneVector{Int,oneL0.SharedBuffer}(undef,bsize)

            fill!(m_buf,m)
            fill!(n_buf,n)
            fill!(k_buf,k)
            fill!(alpha_buf,alpha)
            fill!(lda_buf,lda)
            fill!(ldb_buf,ldb)
            fill!(ldc_buf,ldc)
            fill!(beta_buf,beta)
            fill!(groupsize_buf,1)

            m_ptr = unsafe_wrap(Array,m_buf)
            n_ptr = unsafe_wrap(Array,n_buf)
            k_ptr = unsafe_wrap(Array,k_buf)
            alpha_ptr = unsafe_wrap(Array,alpha_buf)
            lda_ptr = unsafe_wrap(Array,lda_buf)
            ldb_ptr = unsafe_wrap(Array,ldb_buf)
            ldc_ptr = unsafe_wrap(Array,ldc_buf)
            beta_ptr = unsafe_wrap(Array,beta_buf)
            groupsize_ptr = unsafe_wrap(Array,groupsize_buf)

            queue = global_queue(context(A[1]), device(A[1]))
            $fname(sycl_queue(queue), transA, transB, m_ptr, n_ptr, k_ptr, alpha_ptr, Aptrs, lda_ptr, Bptrs,
                   ldb_ptr, beta_ptr, Cptrs, ldc_ptr, length(A), groupsize_ptr)
            unsafe_free!(Cptrs)
            unsafe_free!(Bptrs)
            unsafe_free!(Aptrs)
            C
